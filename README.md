...
A custom Python port scanner built with `concurrent.futures` and `socket` scans all well-known ports (1-1024) of the public IP. The pipeline fails if any port other than 80 and 443 is detected as open — ensuring no service is accidentally exposed to the internet.

The scan is intentionally limited to ports 1-1024 to avoid false positives caused by ephemeral ports generated by GitHub Actions runners. Ports above 1024 are protected by the reverse proxy architecture — NPM is the only public entry point and forwards nothing outside of 80 and 443.
...
The custom scanner built with Python's `socket` and `concurrent.futures` modules has zero external dependencies — Python is already available on every GitHub Actions runner. Every line of code is visible in the repository, fully auditable, and easy to extend.

The scan covers ports 1-1024 (well-known ports) rather than the full 65535 range. This is a deliberate tradeoff — GitHub Actions runners generate ephemeral ports above 32768 that would cause false positives on a full scan. Ports above 1024 are protected by the reverse proxy architecture regardless: NPM is the sole public entry point and exposes nothing beyond 80 and 443. A full scan would require a self-hosted runner with a static IP to be reliable.
